...
configureNAT() {
  ...
  if ! nft add rule ip nat POSTROUTING oifname "$VM_NET_DEV" counter masquerade; then
    error "$tables" && return 1
  fi

  # shellcheck disable=SC2086
  if ! nft add rule ip nat PREROUTING iifname \"$VM_NET_DEV\" ip protocol tcp ip daddr \"$IP\" tcp ${exclude} counter dnat to "$VM_NET_IP"; then
    error "Failed to configure NF tables!" && return 1
  fi

  if ! nft add rule ip nat PREROUTING iifname \"$VM_NET_DEV\" ip protocol udp ip daddr "$IP" counter dnat to "$VM_NET_IP"; then
    error "Failed to configure NF tables!" && return 1
  fi

  if (( KERNEL > 4 )); then
    # Hack for guest VMs complaining about "bad udp checksums in 5 packets"
    nft \# -A POSTROUTING -t mangle -p udp --dport bootpc -j CHECKSUM --checksum-fill > /dev/null 2>&1 || true
  fi
